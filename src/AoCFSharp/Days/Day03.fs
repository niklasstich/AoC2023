...
let findGearRatios (symbols: SymbolPosition array) (numbers: NumberPosition array) =
    let numsNextToGears (numbers: NumberPosition array) (symbol: SymbolPosition) =
        crossProductArray [| symbol |] numbers
        |> Array.map (fun ((ax, ay, _), (bx, by, n)) -> ((ax, ay), (bx, by, n)))
        |> Array.filter areNeighbors
        |> Array.map snd
    ...
    symbols
    |> Array.map (numsNextToGears numbers)
